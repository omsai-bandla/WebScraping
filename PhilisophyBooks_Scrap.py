...
response = requests.get("https://books.toscrape.com/catalogue/category/books/philosophy_7/index.html")
print(response.status_code)
soup = BeautifulSoup(response.content, 'html.parser')
print(soup)
#For Philosophy Books Titles
names = []
for i in range(1, 11):
    title = soup.find_all('h3')[i-1].find('a')['title']
    names.append(title)
print(names)
#For Philosophy Books Prices
prices = []
for i in range(1, 11):
    price = soup.find_all('p', class_='price_color')[i-1].text
    prices.append(price)
print(prices)
#For Philosophy Books Ratings
ratings = []
for i in range(1, 11):
    rating = soup.find_all('p', class_='star-rating')[i-1]['class'][1]
    ratings.append(rating)
print(ratings)
#For Links of Philosophy Books
links = []
for i in range(1, 11):
    link = soup.find_all('h3')[i-1].find('a')['href']
    full_link = "https://books.toscrape.com/catalogue/category/books/philosophy_7/" + link
    links.append(full_link)
print(links)
#For images of Philosophy Books
images = []
for i in range(1, 11):
    img = soup.find_all('div', class_='image_container')[i-1].find('img')['src']
    full_img_link = "https://books.toscrape.com/catalogue/category/books/philosophy_7/" + img.replace('../..', '')
    images.append(full_img_link)
print(images)
#For review availability of Philosophy Books
reviews = []
for i in range(1, 11):
    review = soup.find_all('p', class_='instock availability')[i-1].text.strip()
    reviews.append(review)
print(reviews)
#For UPC of Philosophy Books\
upcs = []
for i in range(1, 11):
...
df.to_csv('philosophy_books.csv', index=False)
