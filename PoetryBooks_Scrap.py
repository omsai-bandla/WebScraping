...
response = requests.get("https://books.toscrape.com/catalogue/category/books/poetry_23/index.html")
print(response.status_code)
soup = BeautifulSoup(response.content, 'html.parser')
print(soup)
#For Poetry Books Titles
names = []
for i in range(1, 18):
    title = soup.find_all('h3')[i-1].find('a')['title']
    names.append(title)
print(names)
#For Poetry Books Prices
prices = []
for i in range(1, 18):
    price = soup.find_all('p', class_='price_color')[i-1].text
    prices.append(price)
print(prices)
#For Poetry Books Ratings
ratings = []
for i in range(1, 18):
    rating = soup.find_all('p', class_='star-rating')[i-1]['class'][1]
    ratings.append(rating)
print(ratings)
#For Links of Poetry Books
links = []
for i in range(1, 18):
    link = soup.find_all('h3')[i-1].find('a')['href']
    full_link = "https://books.toscrape.com/catalogue/category/books/poetry_23/" + link
    links.append(full_link)
print(links)
#For images of Poetry Books
images = []
for i in range(1, 18):
    img = soup.find_all('div', class_='image_container')[i-1].find('img')['src']
    full_img_link = "https://books.toscrape.com/catalogue/category/books/poetry_23/" + img.replace('../..', '')
    images.append(full_img_link)
print(images)
#For review availability of Poetry Books
reviews = []
for i in range(1, 18):
    review = soup.find_all('p', class_='instock availability')[i-1].text.strip()
    reviews.append(review)
print(reviews)
#For UPC of Poetry Books
upcs = []
for i in range(1, 18):
...
df.to_csv('poetry_books.csv', index=False)
